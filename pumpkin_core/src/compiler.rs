...
impl Compiler {
        ...
        match stmt {
             ...
             Statement::AssignStmt(s) => {
                 match &s.target {
                     ...
                     _ => return Err(PumpkinError::runtime("Complex assignment (only var/index) supported", s.loc.clone())),
                 }
             },
             Statement::ExprStmt(s) => {
                 self.compile_expr(&s.expression)?;
                 self.emit_op(OpCode::Pop, s.expression.loc().as_ref().map_or(0, |l| l.line));
             },
             ...
             Statement::RepeatStmt(s) => {
                 // Desugaring repeat N { body } is harder without locals (loop variable).
                 // Skip for now.
                 return Err(PumpkinError::runtime("Repeat statements not implemented in bytecode compiler yet", s.loc.clone()));
             },
             ...
             Statement::ImportStmt(s) => {
                 ...
                 let mod_name_idx = self.chunk.add_constant(PumpkinValue::String(std::rc::Rc::new(s.module.clone())));
                 let line = s.loc.as_ref().map_or(0, |l| l.line);
                 ...
                 let var_idx = self.chunk.add_constant(PumpkinValue::String(std::rc::Rc::new(var_name)));
             ...
             Statement::ExportStmt(s) => {
                 ...
                 let name = match &*s.declaration {
                     ...
                     _ => return Err(PumpkinError::runtime("Only Let and Func can be exported", s.loc.clone())),
                 };
                 
                 let name_idx = self.chunk.add_constant(PumpkinValue::String(std::rc::Rc::new(name.clone())));
                 let line = s.loc.as_ref().map_or(0, |l| l.line);
          ...
          // ...
            Expression::ArrayLiteral(a) => {
                ...
                if count > u16::MAX as usize {
                     return Err(PumpkinError::runtime("Array literal too large", a.loc.clone()));
                }
            ...
            Expression::CallExpr(c) => {
                ...
                self.emit_byte(c.arguments.len() as u8, c.loc.as_ref().map_or(0, |l| l.line));
            },
            Expression::Identifier(id) => {
            ...
            Expression::Literal(l) => {
                ...
                if l.value.is_number() {
                    ...
                    let val = PumpkinValue::String(std::rc::Rc::new(l.value.as_str().unwrap().to_string()));
                    self.emit_constant(val, line);
                } else if l.value.is_nil() {
                    self.emit_op(OpCode::Nil, line);
                } else {
                    return Err(PumpkinError::runtime("Unsupported literal type", l.loc.clone()));
                }
            },
            // ...
    }
    ...
    // In struct: params: HashMap<String, usize>
            ...
            name,
                ...
                match b.operator.as_str() {
                    ...
                    "<" => self.emit_op(OpCode::Less, line),
                     _ => return Err(PumpkinError::runtime("Operator compiler not impl", b.loc.clone())),
                }
            },
            // Logic And/Or require short circuiting jumps
            _ => return Err(PumpkinError::runtime("Expr not supported in compiler yet", expr.loc().clone())),
        }
        Ok(())
    }
    ...
    fn identifier_constant(&mut self, name: &Identifier) -> usize {
        self.chunk.add_constant(PumpkinValue::String(std::rc::Rc::new(name.name.clone())))
    }
