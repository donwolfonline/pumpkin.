...
use crate::ast::{Block, SourceLocation};
...
impl fmt::Display for PumpkinValue {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            PumpkinValue::Function(func) => write!(f, "<fn {}>", func.name),
            PumpkinValue::List(l) => {
                let v = l.borrow();
                write!(f, "[")?;
...
impl PumpkinValue {
    ...
    pub fn add(&self, other: &Self) -> Result<PumpkinValue, PumpkinError> {
        match (self, other) {
            ...
            (PumpkinValue::String(a), PumpkinValue::String(b)) => {
                Ok(PumpkinValue::String(Rc::new(format!("{}{}", a, b))))
            }
            (PumpkinValue::String(a), b) => {
                Ok(PumpkinValue::String(Rc::new(format!("{}{}", a, b))))
            }
            (a, PumpkinValue::String(b)) => {
                Ok(PumpkinValue::String(Rc::new(format!("{}{}", a, b))))
            }
    ...
    pub fn div(&self, other: &Self) -> Result<PumpkinValue, PumpkinError> {
        match (self, other) {
            (PumpkinValue::Number(a), PumpkinValue::Number(b)) => {
                if *b == 0.0 {
                    return Err(PumpkinError::DivisionByZeroError { location: None });
                }
