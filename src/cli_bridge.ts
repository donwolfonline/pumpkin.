...
export async function executePumpkinScript(filePath: string) {
    console.log(`🎃 Executing ${filePath} via Rust Core...`);

    // 1. Locate WASM binary
    // In a real package, this path resolution needs to be robust (e.g. using require.resolve)
    ...
    if (result_js.success) {
        ...
        if (result_js.output && Array.isArray(result_js.output)) {
            result_js.output.forEach((line: string) => console.log(line));
        }
    } else {
